...
export function formatReport(rep) {
  ...
  L.push('');
  L.push('1️⃣ GROUP HEALTH: ' + o.healthScore + '/100 (' + o.healthLabel + ')');
...
function aSBU(d, rep, x) {
  ...
  L.push('   • Stock value: ' + fmt(x.v) + ' BDT (' + (x.v >= 0 ? 'Positive' : 'NEGATIVE ⚠️') + ')');
  ...
  if (prRow) L.push('   • PR pending: ' + prRow.q.toLocaleString() + ' qty across ' + prRow.n.toLocaleString() + ' PRs (last: ' + prRow.d + (sc.prStale ? ' — STALE ⚠️' : '') + ')');
...
function aAging(d, rep) {
  ...
  const L = ['⏳ IDLE INVENTORY ANALYSIS'];
  ['Obsolete', 'Non-Moving', 'Slow-Moving'].forEach(cat => {
    ...
    rows.forEach(x => L.push('    - ' + x.s + ': ' + fmt(x.v) + ' (' + x.d + 'd idle)' + (x.v < 0 ? ' ⚠️ negative value' : '')));
  });
  L.push('');
...
function aDIO(d, rep) {
  ...
  rows.forEach(r => {
    const flag = r.dioDays > 365 ? ' ⚠️ HIGH' : r.dioDays > 120 ? ' ⚠️ watch' : ' ✓';
...
function aPR(d) {
  ...
  pr.forEach((x, i) => {
    ...
    L.push((i + 1) + '. ' + x.s + ': ' + x.q.toLocaleString() + ' qty | ' + x.n.toLocaleString() + ' PRs | ' + x.it.toLocaleString() + ' items | last: ' + x.d + (ds > 90 ? ' ⚠️ stale (' + ds + 'd)' : ''));
  });
...
function aRisks(rep) {
  const L = ['⚠️ TOP RISKS (by severity)'];
  ...
  if (!rep.risks.length) L.push('No material risks detected. ✓');
  return L.join('\n');
}
...
export function askAgent(d, q) {
  ...
  let intent = 'summary', answer = aSummary(d, rep);

  const sbu = findSBUInQuestion(ql, d.stock || []);
  if (sbu && !/^(help|what can you)/.test(ql)) {
    intent = 'sbu-profile';
    answer = aSBU(d, rep, sbu);
  } else if (/help|what can you do|capabilities/.test(ql)) {
    intent = 'help'; answer = HELP;
  } else if (/negative|below zero|-?\bcr\b.*negative/.test(ql)) {
